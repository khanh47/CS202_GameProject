...
public:
    ...
    using PlayerCreator = std::function<std::shared_ptr<GameObject>(sf::Texture* texture, const std::string& animationSetId)>;

    GameObjectFactory();

    void registerPlayer(const std::string& key, PlayerCreator creator);
    void registerBlock(const std::string& key, Creator creator);
    void registerEnemy(const std::string& key, PlayerCreator creator);
    ...
    std::shared_ptr<GameObject> createEnemy(const std::string& key = "Enemy", sf::Texture* texture = nullptr, const std::string& animationSetId = "goomba") const;
    std::shared_ptr<GameObject> createItem(const std::string& key = "Item", sf::Texture* texture = nullptr) const;
private:
    std::unordered_map<std::string, PlayerCreator> _playerCreators;
    std::unordered_map<std::string, Creator> _blockCreators;
    std::unordered_map<std::string, PlayerCreator> _enemyCreators;
