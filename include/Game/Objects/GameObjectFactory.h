...
public:
    ...
    void registerProp(const std::string& key, Creator creator);
    ...
    std::shared_ptr<GameObject> createProp(const std::string& key = "Prop", sf::Texture* texture = nullptr) const;
private:
    ...
    std::unordered_map<std::string, Creator> _propCreators;
};
